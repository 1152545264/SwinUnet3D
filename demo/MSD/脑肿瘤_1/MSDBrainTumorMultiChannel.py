...
class Config(object):
    ...
    n_classes = 3 * 2  # 分割总的类别数，在三个通道上分别进行前景和背景的分割，三个通道为：TC（肿瘤核心）、WT（整个肿瘤）和ET（肿瘤增强)。
...
class LitsDataSet(pl.LightningDataModule):
    ...
    def get_preprocess(self):
        self.train_process = Compose([
            ...
            transforms.SpatialPadd(keys=['image', 'seg'], spatial_size=cfg.PadShape),
...
class LITSModel(pl.LightningModule):
    def __init__(self, cfg=Config()):
        ...
        self.loss_func = DiceFocalLoss(smooth_nr=0, smooth_dr=1e-5, squared_pred=True, to_onehot_y=True, sigmoid=True,
    ...
    def shared_step(self, y_hat, y):
        ...
        y_tc, y_wt, y_et = torch.unsqueeze(y_tc, dim=1), torch.unsqueeze(y_wt, dim=1), torch.unsqueeze(y_et, dim=1)

        y_tc_hat, y_wt_hat, y_et_hat = y_hat[:, 0:2, ::], y_hat[:, 2:4, ::], y_hat[:, 4:, ::]
        y_tc_hat, y_wt_hat, y_et_hat = self.final_act(y_tc_hat), self.final_act(y_wt_hat), self.final_act(y_et_hat),
        ...
        # B,C,H,W,D
        y_tc_hat = torch.argmax(y_tc_hat, dim=1, keepdim=True)
        y_wt_hat = torch.argmax(y_wt_hat, dim=1, keepdim=True)
        y_et_hat = torch.argmax(y_et_hat, dim=1, keepdim=True)

        tc_dice = self.metrics(y_tc_hat, y_tc).squeeze(dim=1)
        wt_dice = self.metrics(y_wt_hat, y_wt).squeeze(dim=1)
        et_dice = self.metrics(y_et_hat, y_et).squeeze(dim=1)
        # dices = torch.stack([tc_dice, wt_dice, et_dice], dim=0)

        dices = torch.cat((tc_dice, wt_dice, et_dice))
